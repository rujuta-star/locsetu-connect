...
export default function SosPage() {
  ...
  const getLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        toast({ title: "Location detected", description: "Your coordinates have been captured." });
      }, () => {
        toast({ title: "Cannot access location", description: "Please enter location manually.", variant: "destructive" });
      });
    }
  };
  ...
  const sendSOS = async () => {
    ...
    try {
      ...
      toast({ title: "SOS failed", description: "Please try again or call 112.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <div className="bg-gradient-to-br from-red-700 to-rose-800 text-white py-10 px-4">
        <div className="max-w-lg mx-auto text-center">
          ...
          <h1 className="text-3xl font-black mb-2">SOS Safety Alert</h1>
          <p className="text-red-100">Send an emergency alert to admin and your contacts</p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-8 flex-1 space-y-5">
        {sent ? (
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <h2 className="text-2xl font-black mb-2">SOS Alert Sent!</h2>
            <p className="text-muted-foreground mb-6">Help is on the way. Admin and your contacts have been notified.</p>
            <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-left space-y-3">
              <p className="text-sm font-medium text-green-800">What to do now:</p>
              <ul className="space-y-2 text-sm text-green-700">
                <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Stay in a safe location</li>
                <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Keep your phone charged</li>
                <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Call 112 if immediate danger</li>
              </ul>
            </div>
            <Button onClick={() => setSent(false)} variant="outline" className="mt-6 rounded-xl">Send Another Alert</Button>
          </motion.div>
        ) : (
          <>
            {/* National Emergency Numbers */}
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
              <p className="text-sm font-bold text-red-800 mb-3">National Emergency Numbers</p>
              <div className="grid grid-cols-3 gap-2">
                {[{ label: "Police", num: "100" }, { label: "Ambulance", num: "108" }, { label: "Emergency", num: "112" }].map(e => (
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-muted-foreground">Emergency Contact Number</label>
              ...
              <div className="flex items-center gap-2 justify-center text-sm text-muted-foreground">
                ...
                <span>Your alert is sent to admin and your emergency contact</span>
              </div>
            </div>
          </>
        )}
